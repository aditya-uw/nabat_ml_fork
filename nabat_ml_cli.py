...
class Processor():
    ...
    def process_single_wav(self, file, path):
        ...
        dets.to_csv(f'../Downloads/nb__{Path(file).name.split(".")[0]}.csv')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', "--directory", help="Path to local files to process")

    parser.add_argument('-x', "--pattern", help="Pattern in glob to look for")
    ...
    for filepath in wav_filepaths:
        ...
        single_run(filepath, args.path[0])
